import {
  Backdrop,
  Box,
  Button,
  Fade,
  IconButton,
  Modal,
  TextField,
  Typography,
} from "@mui/material";
import DoneIcon from "@mui/icons-material/Done";
import ClearIcon from "@mui/icons-material/Clear";
import DeleteIcon from "@mui/icons-material/Delete";
import { useEffect, useState } from "react";
import Service from "../../../../models/ServiceModel";
import {
  StyledClearIcon,
  StyledTextFieldModal,
  StyledTypographyModal,
} from "../../../molecules/dashboard/groupPage/styledComponent";

interface Props {
  open: boolean;
  onClose: () => void;
  operation: string;
  handleAddNewItem: (newItem: string) => void;
  handleEditItem: (editedItem: any) => void;
  editItem: Service | null;
  handleDeleteItem: (serviceId: number | undefined) => void;
}

const CustomModal = (props: Props) => {
  const {
    open,
    onClose,
    operation,
    handleAddNewItem,
    editItem,
    handleEditItem,
    handleDeleteItem,
  } = props;

  const [serviceName, setServiceName] = useState("");

  const addClearInputField = (serviceName: string) => {
    handleAddNewItem(serviceName);
    setServiceName("");
  };

  useEffect(() => {
    if (open) {
      setServiceName(editItem ? editItem.serviceName || "" : "");
    }
  }, [open, editItem]);

  const handleServiceChange = (
    e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>,
  ) => {
    e.preventDefault();
    setServiceName(e.target.value);
  };

  const style = {
    position: "absolute" as "absolute",
    top: "50%",
    left: "50%",
    transform: "translate(-50%, -50%)",
    width: operation === "delete" ? 500 : 600,
    bgcolor: "background.paper",
    border: "none",
    boxShadow: 24,
    overflowY: "auto",
    overflowX: "hidden",
    maxHeight: "90vh",
  };

  const isSaveButtonDisabled = serviceName.trim() === "";

  return (
    <Modal
      aria-labelledby="transition-modal-title"
      aria-describedby="transition-modal-description"
      open={open}
      closeAfterTransition
      slots={{ backdrop: Backdrop }}
      slotProps={{
        backdrop: {
          timeout: 500,
        },
      }}
    >
      <Fade in={open}>
        <Box sx={style}>
          <Box
            sx={{
              backgroundColor: "#005f87",
              color: "white",
              p: 0.2,
              pl: 2,
              fontWeight: "bold",
              display: "flex",
              justifyContent: "space-between",
              alignItems: "center",
            }}
          >
            <Typography component="p" fontWeight="bold">
              {operation === "add"
                ? "Add Service"
                : operation === "edit"
                  ? "Edit Service"
                  : "Confirmation"}
            </Typography>
            <StyledClearIcon
              onClick={() => {
                onClose();
                setServiceName(""); // Reset the input field on close
              }}
            >
              <ClearIcon />
            </StyledClearIcon>
          </Box>
          {operation === "delete" ? (
            <Box sx={{ padding: 2, pb: 3 }}>
              <Typography sx={{ fontSize: "15px", mb: 1 }}>
                Are you sure you want to delete this?
              </Typography>
            </Box>
          ) : (
            <Box sx={{ padding: 2, pb: 6 }}>
              <StyledTypographyModal
                sx={{ fontSize: "15px", fontWeight: "bold", mb: 1 }}
              >
                Service Name
              </StyledTypographyModal>
              <StyledTextFieldModal
                fullWidth
                value={serviceName}
                variant="outlined"
                type="text"
                onChange={handleServiceChange}
              />
            </Box>
          )}
          <Box
            sx={{
              backgroundColor: "#f5f7fa",
              p: 1.5,
              display: "flex",
              justifyContent: "flex-end",
            }}
          >
            <Button
              size="small"
              sx={{ mr: 1, color: "#2b7985", borderColor: "#2b7985" }}
              variant="outlined"
              onClick={() => {
                onClose();
                setServiceName(""); // Reset the input field on close
              }}
            >
              <ClearIcon fontSize="small" sx={{ pr: 0.5, color: "#2b7985" }} />
              Cancel
            </Button>
            <Button
              variant="contained"
              size="small"
              sx={{
                backgroundColor: "#2b7985",
                display: "flex",
                alignItems: "center",
              }}
              onClick={() => {
                if (operation === "add") {
                  addClearInputField(serviceName);
                } else if (operation === "edit" && editItem) {
                  handleEditItem({ ...editItem, serviceName: serviceName });
                } else if (operation === "delete" && editItem) {
                  handleDeleteItem(editItem.serviceId);
                }
              }}
              disabled={isSaveButtonDisabled}
            >
              {operation === "delete" ? (
                <DeleteIcon fontSize="small" sx={{ color: "white", pr: 0.5 }} />
              ) : (
                <DoneIcon fontSize="small" sx={{ color: "white", pr: 0.5 }} />
              )}
              {operation === "add"
                ? "Save"
                : operation === "edit"
                  ? "Save"
                  : "Ok"}
            </Button>
          </Box>
        </Box>
      </Fade>
    </Modal>
  );
};

export default CustomModal;
