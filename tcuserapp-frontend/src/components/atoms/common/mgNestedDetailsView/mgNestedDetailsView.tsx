...
import ManagerCustomModal from "../modals/managerCustomModal";
...
const MgNestedDetailsView = () => {
  ...
  const getRequestDetailsDataById = async (requestId: number) => {
    try {
      const response = await TcManager.getRequestDetailsView(requestId);
      if (response.data) {
  ...
  const approveRquest = async (requestId: number | null, comment: string) => {
    const requestForApproval: CommonRequest = {
      requestId: requestId,
      costApproverComments: comment,
    };
    try {
      const response =
        await TcManager.approveRequestByManager(requestForApproval);
      if (response.data) {
  ...
  const rejectRquest = async (requestId: number | null, comment: string) => {
    ...
    try {
      const response =
        await TcManager.rejectRequestByManager(requestForRejection);
      if (response.data) {
  ...
  const deleteAssignedRole = async (assignedRoleId: number) => {
    try {
      const response =
        await TcManager.deleteAssignedRoleByManager(assignedRoleId);
    ...
    <>
      <Paper sx={{ p: 2, pb: 3 }} elevation={0}>
        ...
        <Grid container item xs={12}>
          ...
          <Grid item xs={5} bgcolor="#ccc" p={2}>
                ...
                >
                  <Table>
                    ...
                    <TableBody>
                      ...
                      <StyledTableRow>
                        <TablePaginationCell colSpan={4} sx={{ width: "100%" }}>
                          <StyledTablePagination
...
export default MgNestedDetailsView;
